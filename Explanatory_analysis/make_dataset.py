...
def save_fasta_from_pdbs(pdb_dir: str, out_dir: str):
    ...
    def prody_read_seq(pdb_path: str, chain_id: str):
        """Reads sequence from PDB, based on carbon-alpha atoms"""
        structure = prody.parsePDB(pdb_path, subset="ca", chain=chain_id)
        seq = structure.select("ca").getSequence()
        return seq
    ...
    for pdb_path in pdbs:
        ...
        chain_id = pdb.split("_")[-1]
        assert len(chain_id) == 1

        seq = prody_read_seq(str(pdb_path), chain_id)
...
class Discotope_Dataset(torch.utils.data.Dataset):
    ...
    def process_samples_parallel(self, n_jobs=8):
        ...
        results = Parallel(n_jobs=n_jobs)(
            delayed(self.process_sample)(i) for i in range(len(self.fasta_dict))
        )
        return results
